			...
			try {
				...
				for (KeyValue kv : columns.list()) {
					...
					for (Object key : json.keySet()) {
						...
						put.add(columnFamily, Bytes.toBytes(key.toString()), // co
																				// ParseJson-1-Put
																				// Store
																				// the
																				// top-level
	...
	public static void main(String[] args) throws Exception {
		...
		if (column != null) {
			...
			if (colkey.length > 1) {
				...
				conf.set("conf.columnfamily", Bytes.toStringBinary(colkey[0])); // co
																				// ParseJson-2-Conf
																				// Store
																				// the
																				// column
		...
		TableMapReduceUtil.initTableMapperJob(input, scan, ParseMapper.class, // co
																				// ParseJson-3-SetMap
																				// Setup
																				// map
																				// phase
		...
		TableMapReduceUtil.initTableReducerJob(output, // co
														// ParseJson-4-SetReduce
														// Configure an identity
														// reducer to store the
														// parsed data.
